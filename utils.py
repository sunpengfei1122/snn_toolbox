...
class AbstractModelParser:
    ...
    def parse(self):
        ...
        for layer in layers:
            ...
            #    print("just come:",inserted_flatten)
            ...
            if layer_type in {'Dense', 'Conv1D', 'Conv2D', 'DepthwiseConv2D'}:
                ...
                #    print("parsing weights:", weights)
                #    print("parsing bias:", bias)

                weights, bias = modify_parameter_precision(
                    weights, bias, self.config, attributes)

                attributes['parameters'] = (weights, bias)
                #print("parsing attrubutes parameters", attributes['parameters'])
                self.absorb_activation(layer, attributes)
    ...
    def build_parsed_model(self):
        ...
        for layer in self._layer_list:
            ...
            #if layer_type == 'Conv2D':
            #    print("build weights:", np.shape(layer['weights'][0]))
            #    print("the weights is :", layer['weights'][0][0,0,0,2])
            #    print("the weights is :", layer['weights'][0][2,2,0,2])
