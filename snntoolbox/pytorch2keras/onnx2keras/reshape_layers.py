...
def convert_flatten(node, params, layers, node_name, keras_name):
    ...
    input_0 = ensure_tf_type(layers[node.input[0]], layers[list(layers)[0]], name="%s_const" % keras_name)
    #print("shape of inpu t0:", np.shape(input_0))
    if params['change_ordering']:
        ...
        reshape = keras.layers.Flatten()
        reshape1 = keras.layers.Reshape([-1], name=keras_name)
        layers[node_name] = reshape1(input_0)
        #print("using reshape:",(reshape1(input_0).shape))
        #print("using flatten:", (reshape(input_0).shape))
